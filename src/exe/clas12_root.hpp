...
size_t run(std::shared_ptr<TChain> _chain, const std::shared_ptr<RootWriter>& _root, int thread_id, const std::string& output_filename) {
  ...
  std::cout << "Branches12 constructed for thread " << thread_id << std::endl;
